...
void CustomLangPack::fetchCustomLangPack(const QString& langPackId, const QString& langPackBaseId) {
	...
	if (!langPackId.isEmpty() && !langPackBaseId.isEmpty() && !needFallback) {
		url.setUrl(qsl("https://nubesurrealista.github.io/Localization/%1.json").arg(langPackId));
	} else {
		url.setUrl(qsl("https://nubesurrealista.github.io/Localization/%1.json").arg(needFallback ? langPackBaseId : langPackId));
	}
