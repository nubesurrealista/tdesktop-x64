	...
	if (checked == MediaCheckResult::Unsupported) {
		...
		if (const auto media = data.vmedia()) {
			...
			if (_media && _media->webpage()) {
				if (isBlocked) {
					_media->webpage()->applyChanges(WebPageType::Article, "", "", "", "", TextWithEntities(), FullStoryId(), nullptr, nullptr, WebPageCollage(), nullptr, nullptr, nullptr, nullptr, 0, 0, "", false, false, 0);
				}
			}
		}
		...
		if (const auto richMessage = data.vrich_message()) {
			...
			auto textWithEntitiesInner = TextWithEntities{
				qs(data.vmessage()),
				Api::EntitiesFromMTP(
					&history->session(),
					data.ventities().value_or_empty())
			};
			textWithEntities = _media ? textWithEntitiesInner : EnsureNonEmpty(textWithEntitiesInner);
		}
		...
		if (GetEnhancedBool("blocked_user_spoiler_mode")) {
			...
			if (blockExist(peerId.value) || (user && user->isBlocked())) {
