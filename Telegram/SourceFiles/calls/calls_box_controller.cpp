...
void BoxController::Row::refreshStatus() {
	...
	auto text = [this] {
		auto time = QLocale().toString(ItemDateTime(_items.front()), GetEnhancedBool("show_seconds") ? QLocale::system().timeFormat(QLocale::LongFormat).remove("t") : QLocale::system().timeFormat(QLocale::ShortFormat));
