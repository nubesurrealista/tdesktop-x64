...
void FiltersMenu::setupList() {
	const auto replaceEditButton = GetEnhancedBool("replace_edit_button");

	_list = _container->add(object_ptr<Ui::VerticalLayout>(_container));
	_list->setAccessibleName(
		replaceEditButton ? tr::lng_saved_messages(tr::now) : tr::lng_filters_title(tr::now));

	if (!replaceEditButton) {
		_setup = prepareButton(
				_container,
				-1,
				{ TextWithEntities{ tr::lng_filters_setup(tr::now) } },
